...
using static winformUI.Common.RegistryExtensions;

namespace winformUI.Bean
{
    public class SystemInfo
    {
        ...
        [DllImport("kernel32")]
        public static extern void GlobalMemoryStatus(ref StorageInfo memibfo);
        [DllImport("kernel32")]
        ...
        public static string GetLocalMachineRegKeyValue(string regePath, string sRegKeyName)
        {
            string sRegValue = string.Empty;
            RegistryKey regKey = null;
            try
            {
                RegistryHiveType registryHiveType = RegistryHiveType.X86;
                if (CommonConstStr.Is64BiteOS)
                {
                    registryHiveType = RegistryHiveType.X64;
                }
                RegistryKey regKeyEx = RegistryExtensions.OpenBaseKey(RegistryHive.LocalMachine, registryHiveType);
                regKey = regKeyEx.OpenSubKey(regePath, false);
                if (!string.IsNullOrEmpty(sRegKeyName))
                {
                    sRegValue = regKey.GetValue(sRegKeyName).ToString();
                }
                regKey.Close();
            }
            catch (Exception ex)
        ...
        public static RegistryKey GetLocalMachineRegKey(string regePath)
        {
            string sRegValue = string.Empty;
            RegistryKey regKey = null;
            try
        ...
        public static RegistryKey GetCurrentUserRegKey(string regePath)
        {
            string sRegValue = string.Empty;
            try
            {
                ...
                RegistryKey regKeyEx = RegistryExtensions.OpenBaseKey(RegistryHive.CurrentUser, registryHiveType);
                return regKeyEx.OpenSubKey(regePath, false);
            }
            catch (Exception ex)
            {
            }
        ...
        public static RegistryKey GetHkeyUsersRegKey(string regePath)
        {
            string sRegValue = string.Empty;
            try
            {
                ...
                RegistryKey regKeyEx = RegistryExtensions.OpenBaseKey(RegistryHive.Users, registryHiveType);
                return regKeyEx.OpenSubKey(regePath, false);
        ...
        public static RegistryKey GetRegistryKey(RegistryHive hive, string path)
        {
            switch (hive)
            {
                case RegistryHive.LocalMachine:
                    return GetLocalMachineRegKey(path);
                    break;
                case RegistryHive.Users:
                    return GetHkeyUsersRegKey(path);
                    break;
                case RegistryHive.CurrentUser:
                    return GetCurrentUserRegKey(path);
                    break;
            }
            return null;
        }
        ...
        //获得系统型号
        public static string get_SystemModel()
        {
            try
            {
                string st = "";
