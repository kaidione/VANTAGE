...
namespace Http.Util
{
    ...
    public class HttpRequest
    {
        ...
        public static string SendPost(string url, string jsonData)
        {
            ...
            {
                Debug.WriteLine("发送GET请求出现异常：" + ex.Message);
                string message = string.Format("The url {0} send {1} error", url, jsonData);
                Logger.Instance.WriteLog($"发送GET请求出现异常: {ex.Message} and the error is {message}", LogType.Error);
            }
            return result;
        }
        ...
        public static string SendPost(string url, string jsonData, string encoding)
        {
            ...
            {
                string message = string.Format("The url {0} send {1} error", url, jsonData);
                Logger.Instance.WriteLog($"发送POST请求出现异常: {ex.Message} and the error is {message}", LogType.Error);
            }
            return result;
        }
