...
namespace LenovoVantageTest.Pages.Helper
{
    class ImageDiff
    {
        public int[] Resize(string imageFile)
        {
            Image img = Image.FromFile(imageFile);
            Bitmap imgOutput = new Bitmap(img, 256, 256);
            int[] histogram = GetHisogram(imgOutput);
        ...
        public float ImageDiffByPIc(string previousImage, string currentImage)
        {
            return GetResult(Resize(previousImage), Resize(currentImage));
        }
    }
}
