Feature: ITSNewOSMultipleClients
	ITS Bamboo Agent 1 To Multiple Client

# http://10.119.186.215/micro/index.html
@ITSNewOSOSNWMultipleClientsSpecFlow
Scenario: Run Micro ITS Local Run 1 Client DYTC4CQL
	Given Run Clients '10.119.186.215' And Run Version 'OldShellNewPlugin=OldShellNewPlugin' Run Cat 'cat=ThinkDytc4CQL || cat=DYTC4CQLSearch'
...
Scenario: Run Micro ITS Local Run 2 Client ITS3
	Given Run Clients '10.119.179.88' And Run Version 'OldShellNewPlugin=OldShellNewPlugin' Run Cat 'cat=ITS3ToolbarNewOS || cat=ITS3UI || cat=ITS3UIS3S4 || cat=ITS3UI_DisconnectNetwork ||cat=ITS3Search'
...
Scenario: Run Micro ITS Local Run 4 Client DYTC6AMT
	Given Run Clients '10.119.159.53' And Run Version 'OldShellNewPlugin=OldShellNewPlugin' Run Cat 'cat=DYTC6AMTUI'
...
Scenario: Run ITS Local Run 8 Client Agent-11 DYTC5
	Given Run Clients '10.119.145.190' And Run Version 'OldShellNewPlugin=OldShellNewPlugin' Run Cat 'cat=DYTC5UI||cat=DYTC5toolbar'
###cat=DYTC5ActiveStateUI||
# http://10.119.146.20/micro/index.html
@ITSNewOSOSNWMultipleClientsSpecFlow
Scenario: Run ITS Local Run 9 Client ITS5GPUOC
	Given Run Clients '10.119.146.20' And Run Version 'OldShellNewPlugin=OldShellNewPlugin' Run Cat 'cat=ideaITS5Function_17172S3S4 || cat=ITS5UI_17173 || cat=ITS5ToolbarNewOS || cat=ITS5Search'
...
Scenario: Run ITS Local Run 11 Client ITS agent-41
	Given Run Clients '10.119.154.173' And Run Version 'OldShellNewPlugin=OldShellNewPlugin' Run Cat 'cat=IntelligentCooling_18037 || cat=IntelligentCooling_18037_offline || cat=ThinkDytc6CS2021S3S4_18037 || cat=CheckBatteryConditionOnThinkPad135W || cat= DYTC6MWSSearch'
...
Scenario: Run ITS Local Run 11 Client ITS agent-46 DYTC6NOAMT
	Given Run Clients '10.119.162.231' And Run Version 'OldShellNewPlugin=OldShellNewPlugin' Run Cat 'cat=DYTC6NOAMTUI||cat=DYTC6NOAMTSearch'
