...
@GamingPlanMultipleClientsSpecFlow @Reboot
Scenario: Run Gaming Local Run 6 Client Agent-175.34
	Given Run Clients '10.119.175.34' And Run Version 'CommonBrand=Common' Run Cat 'cat=GamingHybridModeRestartNeed || cat=PowerPlanBalanceMode || cat=HWInfoAMD'
...
Scenario: Run Gaming Local Run 8 Client Agent-188.123
	Given Run Clients '10.119.188.123' And Run Version 'CommonBrand=Common' Run Cat 'cat=PowerPlanPBPlans || cat=HWInfoAMDOCOFF || cat=GamingMacroKeyS4'
...
Scenario: Run Gaming Local Run 12 Client Agent-175.240
	Given Run Clients '10.119.175.240' And Run Version 'CommonBrand=Common' Run Cat 'cat=PowerPlanQuietPlan || cat=GamingRemoveGiveFeedbackS3 || cat=GamingRemoveGiveFeedbackS4'
...
Scenario: Run Gaming Local Run 16 Client Agent-165.49
	Given Run Clients '10.119.165.49' And Run Version 'CommonBrand=Common' Run Cat 'cat=GamingMemoryOCtestRestartNeed || cat=GamingMemoryOCOfftest || cat=GamingMacroKey15InchKeyboardLayout || cat=GamingOldThermalModeAC || cat=GamingOldThermalModeDC || cat= AutoClose || cat=GamingMacroKey15InchKeyboardLayoutS3 || cat= GamingMacroKey15InchKeyboardLayoutS4 || cat=AutoCloseS3 || cat=AutoCloseS4 || cat=RAMOCSupportedDriverinstalled || cat=AutoCloseS3S4 ||cat=GamingOldThermalModeACS4 || cat=GamingNetworkBoostS4'